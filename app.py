...
with intro_ko:
    st.header("소개")
    st.markdown("노드 커피랩에서 개발 중인 **'디플루이드 오믹스 측정 데이터 기반 생두 경도 예측 모델'**입니다. 현재는 생두 경도 예측 모델의 기본 기능을 테스트하는 단계이며 현 버전 모델로 예측된 경도값은 신뢰하기 어렵습니다. 추후 예측 기능의 향상과 추가 기능을 구현하는 업데이트를 진행할 예정입니다.")
...
with intro_en:
    ...
    st.markdown("This is the **'Deefluid Omix Data-based Green Bean Hardness Prediction Model (beta)'** under development at Node Coffee Lab. This model is currently in a basic function testing phase, and predictions from this version may be unreliable. Future updates are planned to enhance prediction accuracy and implement additional features.")
