...
# Load all necessary files once using Streamlit's cache for efficiency.
# 효율성을 위해 스트림릿의 캐시 기능을 사용하여 모든 필수 파일들을 한번만 불러옵니다.
@st.cache_data
def load_resources():
    try:
...
with intro_ko:
    st.header("소개")
    st.markdown("""
    노드 커피랩에서 개발 중인 **'디플루이드 오믹스 측정 데이터 기반 생두 경도 예측 모델'**입니다. 현재는 생두 경도 예측 모델의 기본 기능을 테스트하는 단계이며, 추후 미구현된 기능을 업데이트할 예정입니다.
    """)
    
    st.subheader("모델의 목표와 배경")
    st.markdown("""
    생두의 경도는 로스팅 중 발열 반응, 1차 크랙, 세포벽 분해와 관련되는 매우 중요한 요소입니다. 하지만 이를 직접 측정하는 장비가 없어 객관적인 데이터로 활용하기 어려웠습니다. 본 예측 모델은 생두의 측정 데이터와 이력 정보를 토대로 경도를 예측하여, 이를 로스팅 변수로 활용할 수 있도록 하는 것을 목표로 합니다.
    """)

    st.warning("""
    **주의사항**: 경도는 밀도, 수분, 재배 고도 등 여타 모든 요소와 구분되는 독립된 변수이므로 예측 정확성은 100%가 될 수 없습니다. 예측된 경도 정보는 **보조적인 참고 자료**로만 활용해 주시기를 바랍니다.
    """)

    with st.expander("데이터 측정 방법 보기"):
        st.subheader("계량 방법")
        st.markdown("""
        예측 모델의 기반 데이터는 디플루이드 코리아에서 개발/판매하는 분배기를 사용하여 측정된 값입니다. 오믹스에 기본으로 포함된 자를 사용하여 측정하는 경우에는, 밀도값에 **1.065**를 곱하여 입력해 주세요.
        1. 샘플 컨테이너로부터 약 10cm 높이에서 생두가 소복하게 쌓이도록 붓습니다.
        2. 분배기를 올린 다음 화살표에 손가락을 걸고 살짝 눌러가며 5회 이상, 생두가 움직이지 않을 때까지 돌립니다.
        3. 자를 비스듬하게 세워 동-서-남-북으로 밀어 평탄하게 정돈합니다. 이때 자는 컨테이너에 닿은 상태를 유지해야 합니다.
...
with intro_en:
    ...
    st.markdown("""
    This is the **'Deefluid Omix Data-based Green Bean Hardness Prediction Model (beta)'** under development at Node Coffee Lab. This is the initial testing phase for the model's basic functions. Additional features will be implemented in future updates.
    """)
    
    st.subheader("Model Goal and Background")
    st.markdown("""
    Green bean hardness is a critical factor related to exothermic reactions, first crack, and cell wall degradation during roasting. However, the lack of direct measurement equipment has made it difficult to use hardness as an objective variable. This model aims to estimate green bean hardness based on physical measurements and historical data, making it a usable variable for roasting.
    """)
    
    st.warning("""
    **Disclaimer**: Since hardness is an independent variable distinct from density, moisture, altitude, and other factors, the prediction accuracy cannot be 100%. Please use the predicted hardness information as **supplementary reference material only.**
    """)
    
    with st.expander("See Measurement Guide"):
        st.subheader("Measurement Method")
        st.markdown("""
        The model's base data was measured using a distributor from Deefluid Korea. If using the default Omix ruler, please multiply the measured density value by **1.065** before input.
...
with col1:
    ...
    volume = st.number_input("부피 / Volume (mm³)", value=None, placeholder="170", format="%d")
    
    st.header("부가 정보 / Extra Information")
    altitude = st.number_input("재배 고도 / Cultivation Altitude (m)", value=None, placeholder="1650", format="%d")
...
with col2:
    ...
    st.number_input("측정 데이터 중 부피 밀도 최대값 / Max Bulk Density", disabled=True)
    st.number_input("측정 데이터 중 부피 밀도 최소값 / Min Bulk Density", disabled=True)
    st.text_input("측정 데이터 중 부피 밀도 중간값 범위 / Median Bulk Density", disabled=True)
    st.number_input("측정 데이터 중 수분 최대값 / Max Moisture Content", disabled=True)
    st.number_input("측정 데이터 중 수분 최소값 / Min Moisture Content", disabled=True)
...
if st.button("Predict Hardness / 경도 예측하기", use_container_width=True, type="primary"):
    ...
    if any(v is None for v in [density, humidity, volume, altitude]):
        ...
        std_density_val = train_mean['Omix bulk Density'].std() # Calculate std from train_mean object
        ...
        input_data = {
            'Omix Humidity': humidity, 'Omix bulk Density': density, 'Omix Volume': volume, 'Altitude': altitude,
